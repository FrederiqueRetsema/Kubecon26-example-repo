...
aws cloudformation deploy --stack-name "${STACKNAME}" --template-file "./cloudformation.yaml" --parameter-overrides ConsultantName="${CONSULTANT_NAME}" BranchName="${BRANCH_NAME}" --capabilities "CAPABILITY_NAMED_IAM" --s3-bucket "${BUCKET_NAME}" --profile "${PROFILE}"
